...
func fight_boss(p1 *Character, enemy Character) bool {
	var text_pass rune
	fmt.Println("========== COMBAT ==========")
	fmt.Println()
	...
	for {
		...
		if enemy.Health <= enemy.Max_health/2 && enemy.Strength == 20{
			fmt.Println("Ganon : Tu n'est donc pas arriver ici pour rien.")
			fmt.Scan(&text_pass)
			fmt.Println("Ganon : RWHAAAA")
			fmt.Println("Navi : Oh non, son corp se regenere et devient encore plus grand qu'avant")
			fmt.Println()
		...
		if enemy.Health <= 0 {
			fmt.Println("Ganon : Keugh")
			fmt.Scan(&text_pass)
			fmt.Println("Ganon : ...")
			fmt.Scan(&text_pass)
			fmt.Println("Ganon : Ne t'inquiete pas ...")
			fmt.Scan(&text_pass)
			fmt.Println("Ganon : Je reviendrais plus fort que jamais ...")
			fmt.Scan(&text_pass)
			fmt.Println("Ganon : Et Hyrule sombrera dans la terreur !")
			fmt.Scan(&text_pass)
		...
		if p1.Health <= 0 {
			...
			fmt.Scan(&text_pass)
			fmt.Println("Ganon : Idiot, tu pensais être de taille face a MOI, GANON, AHAHAHAH !!!!")
			fmt.Println()
			return false
		}
	}
